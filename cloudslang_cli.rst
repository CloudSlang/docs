...
1. `Download <https://github.com/CloudSlang/cloud-slang/releases/download/cloudslang-0.8.0/cslang-cli-with-content.zip>`__ the CLI zip file.
...
names to values. Input files end with the .yaml or .yml extensions. If
multiple input files are being used and they contain an input with the
same name, the input in the file that is loaded last will overwrite the
...
in a file named ``execution.log``. The log file stores all the
