...
populated with ``null``. + From data values set in the
`ExecutionSteps <#executionstep>`__ during the creation of the
`ExecutionPlan <#executionplan>`__.
...
A language built upon Score can add events during run time using the
